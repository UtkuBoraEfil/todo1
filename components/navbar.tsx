...
export default function Navbar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className={cn(
        "bg-light-gray rounded-r-lg  shadow-lg  pt-5 flex flex-col justify-between pl-2 lg:pl-3 z-10 transition-all duration-300 ease",
        open === "false" ? "w-0" : "w-auto"
      )}
    >
      <div
        className={cn(
      ...
      >
        <div className="flex flex-col gap-5 pr-2 lg:pr-3">
          <div className="flex justify-between mb-10 gap-1">
            <Logo />
            {isLoading ? null : <CloseNavbar open={open} setOpen={setOpen} />}
          </div>
          <Favorites />
          {children}
        </div>
